...
ssh -i "$KeyFile" "$EC2User@$EC2IP" $setupScript
...
if sudo systemctl list-units --all | grep -q quiz-backend; then
    sudo systemctl restart quiz-backend
    echo "Backend service restarted"
else
    echo "Note: Backend service not configured yet. Follow AWS_DEPLOYMENT.md Step 5"
fi

# Restart Nginx if it exists
if command -v nginx > /dev/null 2>&1; then
    sudo systemctl restart nginx 2>/dev/null && echo "Nginx restarted" || echo "Note: Nginx not configured yet"
fi
'@

ssh -i "$KeyFile" "$EC2User@$EC2IP" $restartScript
Write-Host ""
